...
$.fn.sleekslider = function(object) {
	...
	if (object.thumbs != undefined || object.thumbs != null) {
		if (object.thumbs.length > 1) {
			...
			$prevThumbnail.attr('src', '~/Content/update(5-10)/DOD/images/' + thumbs[thumbs.length - 1]);
			$prevText.text(labels[thumbs.length - 1]);

			$nextThumbnail.attr('src', '~/Content/update(5-10)/DOD/images/' + thumbs[active]);
...
function animateSlider() {
	...
	if (thumbs != undefined || thumbs != null) {
		if (prevClicked) {
			...
			$prevThumbnail.attr('src', '~/Content/update(5-10)/DOD/images/' + thumbs[value]);
			$prevText.text(labels[value]);
		} else {
		    $prevThumbnail.attr('src', '~/Content/update(5-10)/DOD/images/' + thumbs[active - 1]);
			$prevText.text(labels[active - 1]);
		}

		if (active === (thumbs.length - 1)) {
		    $nextThumbnail.attr('src', '~/Content/update(5-10)/DOD/images/' + thumbs[0]);
			$nextText.text(labels[0]);
		} else {
		    $nextThumbnail.attr('src', '~/Content/update(5-10)/DOD/images/' + thumbs[next]);
