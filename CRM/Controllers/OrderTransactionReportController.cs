    ...
    public class OrderTransactionReportController : Controller
    {
        ...
        public ActionResult GetReport(int page, int pagecount, string fromDate, string toDate, long? shopID)
        {
            try
            {
                ...
                try
                {
                    ...
                    OrderTransactionReportModel = OrderTransactionReportModel.Where(x =>  x.ShopID == (shopID == 0 ? x.ShopID : shopID)).ToList();
                    ...
                    OrderTransactionReportModel = (from COVM in OrderTransactionReportModel
                                                   ...
                                                   select new OrderTransactionReportViewModel
                                                   {
                                                       ...
                                                       ShopFranchiseName = gcs.FirstOrDefault(x => x.COID == gcs.Key.COID).ShopFranchiseName,
                                                       //,
        ...
        public ActionResult Export(string fromDate, string toDate, int option, int print, long? shopID)
        {
            try
            {
                ...
                foreach (var row in OrderReport)
                {
                    ...
                    row.City, row.ShopFranchiseName,row.CreateDate}, false);
                }
        ...
        public List<OrderTransactionReportViewModel> Getdata(string fromDate, string toDate, long? shopID)
        {
            ...
            try
            {
                ...
                OrderTransactionReportModel = BusinessLogicLayer.Helper.CreateListFromTable<OrderTransactionReportViewModel>(dt);
                var testDataByShopId = OrderTransactionReportModel.Where(x =>  x.ShopID == (shopID == 0 ? x.ShopID : shopID)).ToList();
                //var TestDataBYDate = OrderTransactionReportModel.Where(x => EntityFunctions.TruncateTime(x.CreateDate.Date) >= EntityFunctions.TruncateTime(frmd.Date) &&
                //                          x => EntityFunctions.TruncateTime(x.CreateDate.Date) >= EntityFunctions.TruncateTime(tod.Date) && x.ShopID == (shopID == 0 ? x.ShopID : shopID)).ToList();

                //OrderTransactionReportModel = OrderTransactionReportModel.Where(x => x.CreateDate >= frmd.Date &&
