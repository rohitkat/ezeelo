...
namespace Administrator.Controllers
{
    ...
    public class ProductUploadTempController : Controller
    {
        ...
        public JsonResult GetCityByStateId(int stateID)
        {
            ...
            try
            {
                ldistrict = db.Districts.Where(x => x.StateID == stateID).Where(c => c.IsActive == true).ToList();
                foreach (var x in ldistrict)
                {

                    lcity = db.Cities.Where(c => c.DistrictID == x.ID).Where(c => c.IsActive == true).ToList();
        ...
        [SessionExpire]
        public JsonResult GetTaxListByFranchiseId(int franchiseID)
        {
            List<ForLoopClass> TaxIDD = new List<ForLoopClass>();
            try
            {
                TaxIDD = (from TM in db.TaxationMasters
                          ...
                          select new ForLoopClass { Name = TM.Name, ID = TM.ID }).OrderBy(x => x.ID).ToList();


            }
            catch (BusinessLogicLayer.MyException myEx)
            {
                ModelState.AddModelError("Error", "There's Something wrong in filling TaxList Dropdown!!");

                BusinessLogicLayer.ErrorLog.ErrorLogFile(DateTime.UtcNow.AddHours(5.5).ToShortTimeString()
                    + Environment.NewLine + myEx.EXCEPTION_MSG + Environment.NewLine
                    + "[OrderTransactionReportController][POST:GetTaxListByFranchiseId]" + myEx.EXCEPTION_PATH,
                    BusinessLogicLayer.ErrorLog.Module.Administrator, System.Web.HttpContext.Current.Server);
            }
            catch (Exception ex)
            {
                ModelState.AddModelError("Error", "There's Something wrong in filling Tax List Dropdown!!");

                BusinessLogicLayer.ErrorLog.ErrorLogFile(DateTime.UtcNow.AddHours(5.5).ToShortTimeString()
                    + Environment.NewLine + ex.Message + Environment.NewLine
                    + "[OrderTransactionReportController][POST:GetTaxListByFranchiseId]",
                    BusinessLogicLayer.ErrorLog.Module.Administrator, System.Web.HttpContext.Current.Server);
            }


            return Json(TaxIDD.ToList(), JsonRequestBehavior.AllowGet);
        }


        [SessionExpire]
        public JsonResult CheckGSTSelection(string TaxId,long? FranchiseId, long? ShopID)
        {
            try
            {
                if (FranchiseId == 0)
                {
                    FranchiseId = Convert.ToInt64( db.Shops.FirstOrDefault(p => p.ID == ShopID).FranchiseID);
                }
                int count = 0;
                var tax = TaxId.Split(',');
                decimal GSTPart1 = 0;
                ...
                {
                    int TaxationId = Convert.ToInt32(i.ToString());
                    FranchiseTaxDetail obj = db.FranchiseTaxDetails.FirstOrDefault(p => p.TaxationID == TaxationId && p.FranchiseID == FranchiseId && p.IsActive == true);
                    if (obj != null)
                    {
                        if (count == 0)
                        {
            ...
            {
                ModelState.AddModelError("Error", "There's Something wrong in checking Tax details!!");

                BusinessLogicLayer.ErrorLog.ErrorLogFile(DateTime.UtcNow.AddHours(5.5).ToShortTimeString()
                    + Environment.NewLine + myEx.EXCEPTION_MSG + Environment.NewLine
                    + "[OrderTransactionReportController][POST:CheckGSTSelection]" + myEx.EXCEPTION_PATH,
                    BusinessLogicLayer.ErrorLog.Module.Administrator, System.Web.HttpContext.Current.Server);
            }
            catch (Exception ex)
            {
                ...
                BusinessLogicLayer.ErrorLog.ErrorLogFile(DateTime.UtcNow.AddHours(5.5).ToShortTimeString()
                    + Environment.NewLine + ex.Message + Environment.NewLine
                    + "[OrderTransactionReportController][POST:CheckGSTSelection]",
                    BusinessLogicLayer.ErrorLog.Module.Administrator, System.Web.HttpContext.Current.Server);
            }
        ...
        [SessionExpire]
        public JsonResult GetCategoryByMerchantId(int MarchantID)
        {
            List<StateCityFranchiseMerchantViewModel> Category = new List<StateCityFranchiseMerchantViewModel>();
            try
            {
                //var lCategory = (from c in db.Categories
                ...
                //                 where P.PlanCode.Contains("GBMR") && op.OwnerID == MarchantID
                //                 select new StateCityFranchiseMerchantViewModel
                //                 {
                //                     CategoryName = c.Name,
                //                     CategoryID = c.ID
                ...
                {
                    StateCityFranchiseMerchantViewModel SCFM = new StateCityFranchiseMerchantViewModel();
                    SCFM.CategoryID = c.CategoryID;
                    SCFM.CategoryName = c.CategoryName;
                    Category.Add(SCFM);
                }
            }
            catch (BusinessLogicLayer.MyException myEx)
            ...
            return Json(Category.Distinct().OrderBy(x => x.CategoryName).ToList(), JsonRequestBehavior.AllowGet);
        }
            ...
            try
            {
                ...
                var QueryResult = (from TP in db.TempProducts
                                   ...
                                   group new { TSS, TP, CTG, BRD, TPV } by new
                                   {
                                       ...
                                       TP.ApprovalStatus,
                                       TP.ApprovalRemark,
                                       TP.HSNCode,   ///Added by Priti  on 1/11/2018
                                       TP.EANCode     ///Added by Priti  on  1/11/2018
                                   ...
                                   select new ProductUploadViewModel
                                   {
                                       ...
                                       ProductID = g.Key.ID,
                                       ShopName = g.Key.Column3,
                                       ShopID = g.Key.ShopID,
                                       HSNCode = g.Key.HSNCode,  ////Added by priti   on 2/11/2018
                                       EANCode = g.Key.EANCode,  ////Added by priti  on 2/11/2018 
                ...
                foreach (var ReadRecord in QueryResult)
                {
                    ...
                    putvm.HSNCode = ReadRecord.HSNCode;  ///Added by Priti on 1/11/2018
                    putvm.EANCode = ReadRecord.EANCode;  ///Added by Priti on 1/11/2018
                ...
                // return RedirectToAction("Index", PUTVML);
            }
        ...
        public ActionResult Index()
        {
            try
            {
                ...
                {
                    ViewBag.ShopId = new SelectList(db.Shops.OrderBy(x => x.Name).Where(c => c.IsActive == true).ToList(), "ID", "Name", ViewBag.ShopId);
                }
                else
                {
                    ViewBag.ShopId = new SelectList(db.Shops.OrderBy(x => x.Name).Where(c => c.IsActive == true).ToList(), "ID", "Name");
        ...
        public ActionResult Details1(long id, long? ShopID)
        {
            ...
            try
            {
                ...
                productuploadtempviewmodel.HSNCode = TP.HSNCode;    //// Added by Priti on 1/11/2018  
                productuploadtempviewmodel.EANCode = TP.EANCode;    //// Added by Priti on 1/11/2018
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.ddlfranchise = new SelectList((from f in db.Franchises
                                                   join bd in db.BusinessDetails on f.BusinessDetailID equals bd.ID
                                                   where f.IsActive == true && bd.Name != ""
                                                   select new { f.ID, bd.Name }).OrderBy(x => x.Name).ToList(), "ID", "Name");


            ViewBag.ddlMerchant = new SelectList(db.Shops.OrderBy(x => x.Name).Where(c => c.IsActive == true).ToList(), "ID", "Name");



            ViewBag.CategoryID = new SelectList(db.Categories.OrderBy(x => x.Name).ToList().Where(c => c.Level == 3), "ID", "Name");
            ...
            List<NewProductVarient> ci = new List<NewProductVarient> { new NewProductVarient { ID = 0, ColorID = 0, SizeID = 0, DimensionID = 0, MaterialID = 0,IsInclusive=true } };
            ProductUploadTempViewModel productUploadTempViewModel = new ProductUploadTempViewModel();
        ...
        List<HttpPostedFileBase> files_0, List<HttpPostedFileBase> files_1, List<HttpPostedFileBase> files_2, List<HttpPostedFileBase> files_3, List<HttpPostedFileBase> files_4, List<HttpPostedFileBase> files_5, List<HttpPostedFileBase> files_6, List<HttpPostedFileBase> files_7, List<HttpPostedFileBase> files_8, List<HttpPostedFileBase> files_9, List<HttpPostedFileBase> files_10, List<HttpPostedFileBase> files_11, List<HttpPostedFileBase> files_12, List<HttpPostedFileBase> files_13, List<HttpPostedFileBase> files_14, List<HttpPostedFileBase> files_15, string submit, string DisplayProductFromDate1, string textarea, List<CategorySpecificationList> CategorySpecificationList, string ddlMerchant = "", string ddlCategory = "")
        {
            ...
            {
                // long ShopID = GetShopID();
                productuploadtempviewmodel.ShopID = Convert.ToInt64(ddlMerchant);
                long PersonalDetailID = GetPersonalDetailID();
                //--------------------------------------------
                int Proresult = (from f in db.TempProducts
                                 join sp in db.TempShopProducts on f.ID equals sp.ProductID
                                 where (f.Name == productuploadtempviewmodel.Name && sp.ShopID == productuploadtempviewmodel.ShopID)
                                 select new { f }).Count();
                //--------------------------------------------
                        ...
                        if (Proresult == 0)
                        {
                            using (TransactionScope ts = new TransactionScope())
                            {
                                try
                                {
                                    ...
                                    productuploadtempviewmodel.CategoryID = Convert.ToInt32(ddlCategory);
                                    ...
                                    TP.HSNCode = productuploadtempviewmodel.HSNCode;    ///Added by Priti on 1/11/2018
                                    TP.EANCode = productuploadtempviewmodel.EANCode;    ///Added by Priti on 1/11/2018
                                ...
                                catch (BusinessLogicLayer.MyException myEx)
                                {
                                    ...
                                    ViewBag.ddlfranchise = new SelectList(db.BusinessDetails.Where(c => c.BusinessTypeID == 2 && c.IsActive == true).ToList(), "ID", "Name");
                                    ViewBag.ddlMerchant = new SelectList(db.Shops.OrderBy(x => x.Name).Where(c => c.IsActive == true).ToList(), "ID", "Name");
                                    ViewBag.stateID = new SelectList(db.States.OrderBy(x => x.Name).Where(c => c.IsActive == true).ToList(), "ID", "Name");
        ...
        [SessionExpire]
        [Authorize(Roles = "ProductUploadTemp/CanRead")]
        public ActionResult Edit(long id, long? ShopID)
        {
            ...
            try
            {
                ...
                //  ViewBag.ddlCategorySecondID = Convert.ToInt32(pID.ID);
                ...
                // ViewBag.ddlCategoryFirstID = Convert.ToInt32(pID1.ID);
                ...
                productuploadtempviewmodel.Description = TP.Description;
                productuploadtempviewmodel.BrandID = TP.BrandID;
                productuploadtempviewmodel.HSNCode = Convert.ToString(TP.HSNCode);    ////Added by Priti 1/11/2018
                productuploadtempviewmodel.EANCode = Convert.ToString(TP.EANCode);    ////Added by Priti 1/11/2018
                productuploadtempviewmodel.SearchKeyword = TP.SearchKeyword.Trim();
                productuploadtempviewmodel.IsActive = TP.IsActive;
                productuploadtempviewmodel.CreateDate = DateTime.Now;
                productuploadtempviewmodel.CreateBy = PersonalDetailID;
                ...
                List<SelectList> MaterialID = new List<SelectList>();
                List<SelectList> PackUnitID = new List<SelectList>();
                List<SelectList> src1 = new List<SelectList>();
                List<SelectList> ThumbSrc = new List<SelectList>();
                    ...
                    case "Save":
                        using (TransactionScope ts = new TransactionScope())
                        {
                            try
                            {
                                ...
                                TP.Name = productuploadtempviewmodel.Name;
                                TP.HSNCode = productuploadtempviewmodel.HSNCode;    ////Added by Priti on 1/18/2018  
                                TP.EANCode = productuploadtempviewmodel.EANCode;    ////Added by Priti on 1/18/2018
                                TP.CategoryID = productuploadtempviewmodel.CategoryID;
                                TP.WeightInGram = productuploadtempviewmodel.WeightInGram;
                                TP.LengthInCm = productuploadtempviewmodel.LengthInCm;
                                TP.BreadthInCm = productuploadtempviewmodel.BreadthInCm;
                                ...
                                TSP.ShopID = productuploadtempviewmodel.ShopID;
                                TSP.ProductID = TP.ID;
                                TSP.IsActive = true;//productuploadtempviewmodel.IsActive;
                                TSP.DisplayProductFromDate = productuploadtempviewmodel.DisplayProductFromDate;
                                TSP.DeliveryTime = productuploadtempviewmodel.DeliveryTime;
                                TSP.DeliveryRate = productuploadtempviewmodel.DeliveryRate;
                                TSP.TaxRate = productuploadtempviewmodel.TaxRate;
                                        ...
                                        else
                                        {
                                            ...
                                            if (i.TaxationID != null)
                                            {
                                                ...
                                                foreach (var taxID in i.TaxationID)
                                                {
                                                    ...
                                                    if (taxID == PrevTaxID)
                                                    {
                                                        // TempProductTax tempProductTax = db.TempProductTaxes.Find(taxID);
        ...
        public ActionResult ShopLogoDisplay()
        {
            long ShopID = GetShopID();
            string src = ImageDisplay.LoadShopLogo(ShopID, ProductUpload.IMAGE_TYPE.Approved);
            ViewBag.ImageURL = src;
        ...
        [SessionExpire]
        public ActionResult ShopLogoUpload()
        {
            return View();
        }
        ...
        [HttpPost]
        [SessionExpire]
        public ActionResult ShopLogoUpload(HttpPostedFileBase Files)
        {
            ...
            try
            {
                ...
                CommonFunctions.UploadShopLogo(Files, ShopID, ProductUpload.IMAGE_TYPE.Approved);
                string src = ImageDisplay.LoadShopLogo(ShopID, ProductUpload.IMAGE_TYPE.Approved);
                return RedirectToAction("Index", "Home");
            }
            catch (BusinessLogicLayer.MyException myEx)
        ...
        public JsonResult AutoCompleteProName(string term, string ddlMerchant)
        {
            ...
            var result = (from op in db.OwnerPlans
                          ...
                          join c in db.Categories on pcc.CategoryID equals c.ID
                          join r in db.Products on c.ID equals r.CategoryID
                          where op.Plan.PlanCode.StartsWith("GBMR") && c.IsActive == true && op.OwnerID == ShopID
                                      && r.IsActive == true
                          ...
                          select new { Name = r.Name, r.ID }).Distinct().ToList();
            ...
            try
            {
                ...
                if (ProductId == 0)//check Is available in your shop or not
                {
                    if (productLimit > productUploadedinShop)// check Product limit according to plan
                    {
                        ...
                        ViewBag.ddlCategoryFirstID = new SelectList(db.Categories.Where(c => c.Level == 1), "ID", "Name", productuploadtempviewmodel.ddlCategoryFirstID);
                        ViewBag.ddlCategorySecondID = new SelectList(db.Categories.Where(c => c.Level == 2), "ID", "Name", productuploadtempviewmodel.ddlCategorySecondID);
                        ViewBag.DisplayProductFromDate1 = DateTime.Now;
                        ViewBag.BrandID = new SelectList(db.Brands.OrderBy(x => x.Name).Where(c => c.IsActive == true).ToList(), "ID", "Name", productuploadtempviewmodel.BrandID);
                        var FranchiseID = db.Shops.Where(x => x.ID == ShopID).Select(x => x.FranchiseID).FirstOrDefault();
                        var TaxIDD = (from TM in db.TaxationMasters
                                      join FTD in db.FranchiseTaxDetails on TM.ID equals FTD.TaxationID
                        ...
                        foreach (var ii in productuploadtempviewmodel.NewProductVarientS)
                        {
                            ...
                            MaterialID.Add(new SelectList(db.Materials.OrderBy(x => x.Name).Where(c => c.IsActive == true).ToList(), "ID", "Name", ii.MaterialID));
                            PackUnitID.Add(new SelectList(db.Units.OrderBy(x => x.Name).Where(c => c.IsActive == true).ToList(), "ID", "Name", ii.PackUnitID));
                            TaxationID.Add(new MultiSelectList(TaxIDD.OrderBy(x => x.ID).ToList(), "ID", "Name"));
                            if (ii.ColorID == 1)
                            {
        ...
        //                                join pcc in db.PlanCategoryCharges on op.PlanID equals pcc.PlanID
        //                                join c in db.Categories on pcc.CategoryID equals c.ID
        //                                where pcc.IsActive == true && op.IsActive == true && op.OwnerID == productuploadtempviewmodel.ShopID && c.IsActive == true
        //                                && op.Plan.PlanCode.StartsWith("GBMR")
        //                                select new ForLoopClass { Name = c.Name, ID = c.ID }).OrderBy(x => x.Name).ToList();
        ...
        //                return RedirectToAction("Index1", "ProductUploadAproved", new { ShopId = productuploadtempviewmodel.ShopID });
        ...
        [ValidateInput(false)]
        [ValidateAntiForgeryToken]
        public ActionResult GBCatalog1([Bind(Include = "ID,Name,CategoryID,WeightInGram,LengthInCm,BreadthInCm,HeightInCm,Description,BrandID,SearchKeyword,IsActive,CreateDate,CreateBy,ModifyDate,ModifyBy,NetworkIP,DeviceType,DeviceID,ProductVarientID,ColorID,SizeID,DimensionID,MaterialID,TempShopProductID,ShopID,DisplayProductFromDate,Qty,ReorderLevel,ShopStockID,ShopProductID,StockStatus,PackSize,PackUnitID,MRP,WholeSaleRate,RetailerRate,IsInclusiveOfTax,NewProductVarientS,CategoryL_2,Path,pathValue,IsSelect,ShopID")] ProductUploadTempViewModel productuploadtempviewmodel,
            List<HttpPostedFileBase> files_0, List<HttpPostedFileBase> files_1, List<HttpPostedFileBase> files_2, List<HttpPostedFileBase> files_3, List<HttpPostedFileBase> files_4, List<HttpPostedFileBase> files_5, List<HttpPostedFileBase> files_6, List<HttpPostedFileBase> files_7, List<HttpPostedFileBase> files_8, List<HttpPostedFileBase> files_9, List<HttpPostedFileBase> files_10, List<HttpPostedFileBase> files_11, List<HttpPostedFileBase> files_12, List<HttpPostedFileBase> files_13, List<HttpPostedFileBase> files_14, List<HttpPostedFileBase> files_15, FormCollection collection, string submit, string DisplayProductFromDate1, List<CategorySpecificationList> categorySpecificationList)
        {
            if (productuploadtempviewmodel != null && productuploadtempviewmodel.NewProductVarientS.Count != 0)
            {
                foreach (var item in productuploadtempviewmodel.NewProductVarientS)
                {
                    ...
                    if (tax != null)
                    {
                        ...
                        {
                            TempData["GSTSelectionMsg"] = "Select same CGST and SGCT % for varient " + db.Sizes.FirstOrDefault(p => p.ID == item.SizeID).Name;
                            return RedirectToAction("GBCatalog1", new { id = productuploadtempviewmodel.ID, ShopId = productuploadtempviewmodel.ShopID });
                        }
                    }
                }
            ...
            try
            {
                        ...
                        if (i.ProductVarientID == 0)
                        {
                            ...
                            if (i.TaxationID != null)
                            {
                                foreach (var taxID in i.TaxationID)
                                {
                                    ...
                                    ProductTax.IsInclusive = i.IsInclusive;
                ...
                // return RedirectToAction("Index");
                // return RedirectToAction("Index", "ProductUploadAproved");
                return RedirectToAction("Index1", "ProductUploadAproved", new { ShopId = productuploadtempviewmodel.ShopID });
