...
namespace Administrator.Controllers
{
    public class MerchantPlanCategoryChargeController : Controller
    {
        ...
        [SessionExpire]
        [CustomAuthorize(Roles = "MerchantPlanCategoryCharge/CanRead")]
        public ActionResult ApplyChargesToMany(long? ownerId, int? Category_2)
        {
            try
            {
                if (ownerId == null)
                ...
                if (franchiseID != null)
                {
                    ...
                    BusinessLogicLayer.GetPlanCatCharge obj = new BusinessLogicLayer.GetPlanCatCharge();
                    opccList = obj.SelectMerchantPlanCategoryCharge(ownerId, franchiseID);
                    TempData.Remove("OwnerCatList");
                    TempData.Add("OwnerCatList", opccList);
                }

                List<int> category_3 = opccList.Select(x => x.CategoryID).ToList();
        ...
        public ActionResult ApplyChargesToMany(FormCollection frm, long? ownerId, bool IsActive)
        {
            try
            {
                ...
                int.TryParse(frm.GetValue("Category_2").AttemptedValue, out catId);                

                List<OwnerPlanCategoryChargeViewModel> opccList = (List<OwnerPlanCategoryChargeViewModel>)TempData.Peek("OwnerCatList");
                ...
                var category_2 = (from c in db.Categories
                                  where category_3.Contains(c.ID) && c.Level == 3
                                  select new { ID = c.ParentCategoryID, Name = c.Category2.Name }).Distinct().OrderBy(x => x.Name);
                ViewBag.Category_2 = new SelectList(category_2, "ID", "Name");
                ...
                using (var dbContextTransaction = db.Database.BeginTransaction())
                {
                    foreach (OwnerPlanCategoryChargeViewModel opccv in catList)
                    {
                        ...
                        else
                        {
                            ...
                            UpdateProductCharges(opcc.OwnerPlanID, opcc.CategoryID, chargeInPercent, chargeInRupee, isactive);
                        }
                    }
