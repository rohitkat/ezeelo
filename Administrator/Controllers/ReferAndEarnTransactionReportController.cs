...
namespace Administrator.Controllers
{
    ...
    public class ReferAndEarnTransactionReportController : Controller
    {
        ...
        //********Dropdown Binding Code Added by harshada on 23/12/2016**********//
        public void GetCityId()
        {
            var lCityID = (from CT in db.Cities
            ...
            ViewBag.CityID = new SelectList(listdata, "Value", "Text");
        }
        public JsonResult GetFranchiseID(long? CityID)
        {
            var lFranchiseID = (from CT in db.Cities
                                join PC in db.Pincodes on CT.ID equals PC.CityID
                                join FR in db.Franchises on PC.ID equals FR.PincodeID
                                where CT.ID == CityID
                                select new
                                {
                                    ...
                                    Text = FR.ContactPerson
                                }).Distinct().ToList();



            List<SelectListItem> listdata = new List<SelectListItem>();
            foreach (var item in lFranchiseID)
            {
                SelectListItem sli = new SelectListItem();
                sli.Value = item.Value;
                sli.Text = item.Text;
            ...
            return Json(listdata.Distinct().OrderBy(x => x.Value).ToList(), JsonRequestBehavior.AllowGet);

        }
        //********End Dropdown binding Code Added by harshada on 23/12/2016**********//

        //********Added by harshada on 23/12/2016  *******//
        ...
        public ActionResult GetReport(int? page, int option, string SearchUserName, string SearchEmail, string SearchMobile, string SearchSchemeName, int Mode, string FromDate, long? FranchiseID, long? CityID, string ToDate, int print)   //---parameters Added by harshada i.e. Mode,FromDate, ToDate,  FranchiseID,  CityID,print
        {
            ...
            try
            {
                ...
                lEarnAndReferDetailReportViewModel = Getdata(SearchUserName, SearchEmail, SearchMobile, SearchSchemeName,Mode,FromDate1,FranchiseID,CityID,ToDate1);
                TotalCount = lEarnAndReferDetailReportViewModel.Count();
                ViewBag.TotalCount = TotalCount;
                //lEarnAndReferDetailReportViewModel = lEarnAndReferDetailReportViewModel.OrderByDescending(x=>x.ReferredDate).Skip((page - 1) * pageSize).Take(pageSize).ToList(); 
                ViewBag.PageSize = lEarnAndReferDetailReportViewModel.Count;
        ...
        public ActionResult Export(int? page, int option, string SearchUserName, string SearchEmail, string SearchMobile, string SearchSchemeName, int Mode, string FromDate, long? FranchiseID, long? CityID, string ToDate)
        {
            List<EarnAndReferDetailReportViewModel> lEarnAndReferDetailReportViewModel = new List<EarnAndReferDetailReportViewModel>();
            DateTime fromdt = CommonFunctions.GetProperDateTime(FromDate);
            string lFromDate = fromdt.ToShortDateString();
