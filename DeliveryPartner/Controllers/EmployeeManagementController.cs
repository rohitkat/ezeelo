       ...
       + "ErrorLog Controller : EmployeeManagementController" + Environment.NewLine);
        ...
        public ActionResult Index()
        {
            try
            {
                ...
                string UserLoginID = Convert.ToInt64(Session["USER_LOGIN_ID"]).ToString();//Add by Ashish
                long MerchantID = getOwnerIDUsingSession(CommonFunctions.GetPersonalDetailsID(Convert.ToInt64(Session["USER_LOGIN_ID"])));
                //List<Employee> lst = new List<Employee>();//Hide
                //lst = db.Employees.Where(x => x.OwnerID == MerchantID && x.EmployeeCode.Substring(0, 4) == "GBDP").ToList();//Hide
               
                //-- Add by Ashish --//
                var lst = (from e in db.Employees
                           join pd in db.PersonalDetails on e.UserLoginID equals pd.UserLoginID//added
                           ...
                           join r in db.Roles on ur.RoleID equals r.ID//added
                           /*join dba in db.DeliveryBoyAttendances on e.UserLoginID equals dba.UserLoginID into ed //Hide EPOD from Ashish for Live
                           from dba in ed.DefaultIfEmpty()//-- For Left Outer Join --//*/  //Hide EPOD from Ashish for Live
                           where (e.OwnerID == MerchantID && e.EmployeeCode.Substring(0, 4) == "GBDP")
                           && e.IsActive == true && pd.IsActive == true && ur.IsActive == true && r.IsActive == true && ul.IsLocked == false //added
                           /*&& (dba.LoginDateTime == (from dbt in db.DeliveryBoyAttendances where dbt.UserLoginID == e.UserLoginID select dbt.LoginDateTime).Max() //.Take(1) //Hide EPOD from Ashish for Live
                             || dba.LoginDateTime == null)*/
                           select new 
                           {
                               ...
                               IsActive = e.IsActive,
                               DeviceID=r.Name,
                              //// DeviceType = dba.LoginDateTime == null && r.Name == "DELIVERY_EMPLOYEE" ? "Logout" : dba.LogoutDateTime == null && r.Name == "DELIVERY_EMPLOYEE" ? "Login" : dba.LogoutDateTime != null && r.Name == "DELIVERY_EMPLOYEE" ? "Logout" : "NA" //Logout //Hide EPOD from Ashish for Live
                           }).AsEnumerable().Select(x => new Employee //-- For converting LinQ to IEnumerable --//
                           {
                               ...
                               OwnerID = x.OwnerID,
                               IsActive = x.IsActive,
                               ////DeviceType = x.DeviceType,//Hide EPOD from Ashish for Live
                               DeviceID=x.DeviceID
                           }).ToList();
        ...
        public ActionResult Edit(long? id)
        {
            try
            {
                ...
                ViewBag.OwnerType = new SelectList(db.BusinessTypes.Where(x => x.Prefix == "GBDP").ToList(), "ID", "Name");
                ...
                lownerType = objODP.OwnerDetail(db.BusinessTypes.Where(x => x.Prefix == "GBDP").FirstOrDefault().ID, System.Web.HttpContext.Current.Server);
                ...
                long ShopID = db.DeliveryPartners.Where(x => x.BusinessDetailID == BusinessID).FirstOrDefault().ID;
        ...
        private string GenerateEmployeeCode(long? OwnerID)
        {
            ...
            string lcode = ("GBDP" + DateTime.Today.Year + DateTime.Today.Month + DateTime.Today.Day + lCount);
