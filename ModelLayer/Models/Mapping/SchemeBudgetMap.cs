using System.ComponentModel.DataAnnotations.Schema;
using System.Data.Entity.ModelConfiguration;

namespace ModelLayer.Models.Mapping
{
    public class SchemeBudgetMap : EntityTypeConfiguration<SchemeBudget>
    {
        public SchemeBudgetMap()
        {
            // Primary Key
            this.HasKey(t => t.ID);

            // Properties
            this.Property(t => t.NetworkIP)
                .HasMaxLength(15);

            this.Property(t => t.DeviceType)
                .HasMaxLength(50);

            this.Property(t => t.DeviceID)
                .HasMaxLength(50);

            // Table & Column Mappings
            this.ToTable("SchemeBudget");
            this.Property(t => t.ID).HasColumnName("ID");
            this.Property(t => t.ReferAndEarnSchemaID).HasColumnName("ReferAndEarnSchemaID");
            this.Property(t => t.BudgetAmount).HasColumnName("BudgetAmount");
            this.Property(t => t.PreRemainingAmt).HasColumnName("PreRemainingAmt");
            this.Property(t => t.PreUsedAmt).HasColumnName("PreUsedAmt");
            this.Property(t => t.ExpiryDate).HasColumnName("ExpiryDate");
            this.Property(t => t.ActionStatus).HasColumnName("ActionStatus");
            this.Property(t => t.CreateDate).HasColumnName("CreateDate");
            this.Property(t => t.CreateBy).HasColumnName("CreateBy");
            this.Property(t => t.ModifyDate).HasColumnName("ModifyDate");
            this.Property(t => t.ModifyBy).HasColumnName("ModifyBy");
            this.Property(t => t.NetworkIP).HasColumnName("NetworkIP");
            this.Property(t => t.DeviceType).HasColumnName("DeviceType");
            this.Property(t => t.DeviceID).HasColumnName("DeviceID");

            // Relationships
            this.HasRequired(t => t.ReferAndEarnSchema)
                .WithMany(t => t.SchemeBudgets)
                .HasForeignKey(d => d.ReferAndEarnSchemaID);

        }
    }
}
